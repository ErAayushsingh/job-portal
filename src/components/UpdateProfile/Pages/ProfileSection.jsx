...
import React, { useState, useEffect } from 'react';
import { Card, CardContent, Typography, Box, Stack, Divider, IconButton, TextField, Button } from '@mui/material';
import { Email, Phone, LocationOn, Work, CurrencyRupee, Description, Build, Favorite, Edit, Save, Cancel, Delete, Link } from '@mui/icons-material';
import { apiService } from '../../ApiService';
import apiEndpoints from '../../apiendpoint';
import ConfirmationDialog from '../../Common/ConfirmationDialog';
import CommonSnackbar from '../../CommonSnackbar';
...
function ProfileSection({ userDetails }) {
    ...
    useEffect(() => {
        const fetchUserDetails = async () => {
            try {
                const response = await apiService.get(apiEndpoints.user.details);
                if (response) {
                    setProfileData({
                        ...
                        userExpreance: response.userExpreance || '',
                        userNoticePeriod: response.userNoticePeriod || '',
        ...
        <Card variant="outlined" sx={{ p: 3, mb: 4, boxShadow: 3, borderRadius: 3 }}>
            <CardContent>
                    ...
                    {Object.entries(profileData).map(([key, value]) => (
                        <Box key={key} display="flex" alignItems="center">
                            {editing ? (
                                ...
                                <Typography variant="subtitle1"><strong>{key.replace(/([A-Z])/g, ' $1').trim()}:</strong> {value}</Typography>
                            )}
                        </Box>
                    ))}
                </Stack>
                <Box mt={2} display="flex" justifyContent="flex-end">
                    {editing ? (
                        <>
                            <Button variant="contained" color="success" onClick={handleSave} startIcon={<Save />}>Save</Button>
                            <Button variant="outlined" color="error" onClick={() => setEditing(false)} startIcon={<Cancel />} sx={{ ml: 2 }}>Cancel</Button>
                        </>
                    ) : (
                        <Button variant="contained" color="primary" onClick={handleEdit} startIcon={<Edit />}>Edit</Button>
                    )}
                </Box>
            </CardContent>
        </Card>
